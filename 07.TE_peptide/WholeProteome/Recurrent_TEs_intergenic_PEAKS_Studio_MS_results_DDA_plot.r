...
library(data.table)
# 设置工作目录
setwd("/data2t_2/pathogen_TE_2025_New/14.Whole_proteome/WholeProteome/result-all-61mb/")
...
# 删除指定列
# dia_db.peptides <- dia_db.peptides[, !names(dia_db.peptides) %in% columns_to_remove]

# 定义Area Sample列名
area_columns <- paste0("Area Sample ", 1:2)
# area_columns <- paste0("Area Sample ", 1:6)
# area_columns <- paste0("Area Sample ", 7:12)
# 计算每行Area Sample列中的NA值数量
...
# 计算Mock和Infection组的均值，并添加TE分类信息
# 1. 根据metadata确定样本分组
...
## all 不分时间段 241个 
dia_db.peptides_filtered$Area.log2FC[dia_db.peptides_filtered$Area.log2FC >= 9] <- 9
dia_db.peptides_filtered$Area.log2FC[dia_db.peptides_filtered$Area.log2FC <= -9] <- -9
dia_db.peptides_filtered.hist <- dia_db.peptides_filtered %>% mutate(peptide.logFC = cut(Area.log2FC, seq(from=-10, to=10, by=0.5)))
...
p1 <- ggplot(dia_db.peptides_filtered.hist, aes(x = peptide.logFC, fill = TE_Class)) + 
  ...
  scale_x_discrete(drop = FALSE,
                   breaks = c("(-9.5,-9]",  "(-8,-7.5]",  "(-7.5,-7]", "(-7,-6.5]" , "(-5,-4.5]", "(-4.5,-4]", 
                              "(-4,-3.5]", "(-3.5,-3]", "(-3,-2.5]", "(-2.5,-2]","(-2,-1.5]", "(-1.5,-1]", 
                              "(-1,-0.5]", "(-0.5,0]", "(0,0.5]", "(0.5,1]", "(1,1.5]","(1.5,2]", "(8.5,9]"),
                   labels = c("Mock only", "-7.5","-7","-6.5", "-4.5", "-4", 
                              "-3.5", "-3", "-2.5", "-2", "-1.5","-1", 
                              "-0.5", "0", "0.5", "1", "1.5","2", "Infection only")) +
  scale_y_continuous(breaks = seq(0, 110, by = 20), 
                     limits = c(0, 105)) +  
  xlab('Peptide log2 Fold Change') + 
  ylab('Number of Peptides') + 
  geom_vline(xintercept = c(4, 36), colour="grey", linetype=2, alpha=1) + 
  theme(
...
ggsave(p1,file="Recurrent_Up_TE_peptide_distribution.pdf",width = 6,height = 3)
...
p <- ggplot(length_TE_Class_table, aes(x = factor(Length), y = count, fill = TE_Class)) +
  ...
  # ggtitle('24h Peptide Length Distribution by TE Class') +
  # ggtitle('48h Peptide Length Distribution by TE Class') +
  scale_y_continuous(breaks = seq(0, max(aggregate(count ~ Length, length_TE_Class_table, sum)$count), by = 20))
p

ggsave(p,file="Recurrent_Up-TE_peptide_length_distribution.pdf",width = 2,height = 2)
