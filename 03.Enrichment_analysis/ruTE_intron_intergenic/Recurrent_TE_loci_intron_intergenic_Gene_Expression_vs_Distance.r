...
p <- ggplot(relationship_counts, aes(x = distance_range, y = count, group = relationship, color = relationship)) +
  ...
  scale_y_continuous(
    breaks = seq(0, max(relationship_counts$count, na.rm = TRUE) + 100, by = 200),  
    expand = c(0.02, 0)  
  ) +
  scale_x_discrete(
    expand = c(0.01, 0.05)  
...
print(p)
ggsave("/data2t_2/pathogen_TE_2025_New/03.Enrichment_analysis/intron_intergenic/Fig3s_Recurrent_TE_Loci_gt5_intron_intergenic_Gene_Expression_vs_Distance.pdf", p, width = 5, height = 3)

p <- ggplot(relationship_counts, aes(x = distance_range, y = count, group = relationship, color = relationship)) +
  geom_line(size = 0.8) +
  geom_point(size = 2) +
  labs(
    title = "intron and intergenic TE_loci-Gene Expression Relationships with Distance",
    x = "absolute distance to TE instance(bp)",
    y = "Count",
    color = ""
  ) +
  ...
  scale_y_continuous(
    guide = "axis_minor",
    breaks = c(seq(0, 1200, by = 200), 3200, 3400, 3600),  
    minor_breaks = NULL
  )
print(p)
ggsave("/data2t_2/pathogen_TE_2025_New/03.Enrichment_analysis/intron_intergenic/Fig3s_Recurrent_TE_Loci_gt5_intron_intergenic_Gene_Expression_vs_Distance_break.pdf", p, width = 6, height = 3.5)
